...
# load data
rawdata=read.csv("CF_All.csv",header=T)
#rawdata=read.csv("CR_All.csv",header=T)
#rawdata=read.csv("OR_All.csv",header=T)
#rawdata=read.csv("OF_All.csv",header=T)
#rawdata=read.csv("CF_All.csv",header=T)
F_index=c(2,3); #CF #for single,replace 3，3 to...72)
#F_index=c(2,3); #CR#for single,replace 3，3 to...72)
#F_index=c(2,3); #OF#for single,replace 3，3 to...72)
#F_index=c(2,3); #OR#for single,replace 3，3 to...72)
#F_index=c(2,3); #ALL
dim(rawdata)

# data separation
np=45 # 20% testing data
...
np=45 # 20%
    ...
    np45=c(acc.train.all,sen.train.all,spe.train.all,acc.test.all,sen.test.all,spe.test.all);
print(np45)
    np45.sd=c(acc.train.sd,sen.train.sd,spe.train.sd,acc.test.sd,sen.test.sd,spe.test.sd);
print(np45.sd) 
Result_all
 write.table(Result_all,file="cf_6",sep=",",col.names=FALSE,row.names=FALSE,quote=TRUE) # edit files names
